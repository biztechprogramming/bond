...
class ToolRegistry:
    ...
    def get_definitions_for(self, tool_names: list[str]) -> list[dict]:
        ...
        # MCP tool definitions are injected separately via proxy or manager
...
def build_registry() -> ToolRegistry:
    ...
    from .shell_utils import (
        ...
        handle_file_list, handle_file_search_unified,
    )
    ...
    def _make_deprecated(old_name: str, new_name: str, handler):
        ...
        async def wrapper(arguments, context):
            logger.warning("Deprecated tool '%s' called. Use '%s' instead.", old_name, new_name)
            return await handler(arguments, context)
        return wrapper
