...
STDB_MODULE="${BOND_SPACETIMEDB_MODULE:-$(python3 -c "import json; print(json.load(open('$REPO_DIR/bond.json')).get('spacetimedb', {}).get('module', 'bond-core-v2'))" 2>/dev/null || echo "bond-core-v2")}"
STDB_URL="${BOND_SPACETIMEDB_URL:-$(python3 -c "import json; print(json.load(open('$REPO_DIR/bond.json')).get('spacetimedb', {}).get('url', 'http://localhost:18787'))" 2>/dev/null || echo "http://localhost:18787")}"
