...
export default function EnvironmentDashboard({ environment, agents, onNavigate }: EnvironmentDashboardProps) {
  ...
  useEffect(() => {
    ...
    const i1 = setInterval(fetchServers, 30_000);
    const i2 = setInterval(fetchReceipts, 60_000);
    const i3 = setInterval(fetchAlerts, 30_000);
    intervalsRef.current = [i1, i2, i3];

    return () => intervalsRef.current.forEach(clearInterval);
  }, [fetchServers, fetchReceipts, fetchAlerts]);

  const health = servers ? overallHealth(servers) : null;
  ...
  const quickActions: { label: string; view: string }[] = [
    ...
    { label: "View Logs", view: "view-logs" },
    { label: "Check Health", view: "check-health" },
    { label: "View Topology", view: "view-topology" },
    { label: "Agent Settings", view: "agent-settings" },
    { label: "Monitoring Config", view: "monitoring-config" },
  ];
