...
eval "$(bash "$SKILL_DIR/scripts/setup-test-db.sh")"

# --- Common environment ---
export PYTHONPATH="$PROJECT_ROOT"
export BOND_HOME="/tmp/visual-ui-test/bond-home"
export BOND_DATABASE_PATH="/tmp/visual-ui-test/bond-test.db"
...
with sync_playwright() as p:
    ...
    browser.close()
" && echo "✓ Playwright can connect to frontend" || echo "⚠ Playwright connectivity check failed (services may still be starting)"
